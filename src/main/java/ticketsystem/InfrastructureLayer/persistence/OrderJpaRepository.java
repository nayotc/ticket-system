...
public interface OrderJpaRepository extends JpaRepository<ActiveOrder, Long> {
    ...
    @Query("SELECT o FROM ActiveOrder o LEFT JOIN FETCH o.tickets WHERE o.userId = :userId AND o.eventId = :eventId")
    Optional<ActiveOrder> findByUserIdAndEventIdWithTickets(
            @Param("userId") Long userId,
            @Param("eventId") Long eventId
    );

    @Query("SELECT o FROM ActiveOrder o LEFT JOIN FETCH o.tickets WHERE o.sessionToken = :sessionToken AND o.eventId = :eventId")
    Optional<ActiveOrder> findBySessionTokenAndEventIdWithTickets(
    ...
    @Query("""
            ...
            WHERE o.status = ticketsystem.DomainLayer.order.ActiveOrder.OrderStatus.ACTIVE
            AND o.expiresAt <= :now
            """)
    List<ActiveOrder> findExpiredActiveOrders(@Param("now") LocalDateTime now);
    ...
    @Query("""
            ...
            AND o.expiresAt <= :now
            """)
    List<ActiveOrder> findExpiredActiveOrdersByEventId(
            @Param("eventId") Long eventId,
    ...
    @Query("""
            ...
            WHERE o.status = ticketsystem.DomainLayer.order.ActiveOrder.OrderStatus.ACTIVE
            AND o.expiresAt > :now
            AND o.expiresAt <= :warningThreshold
            """)
    List<ActiveOrder> findOrdersAboutToExpire(
            @Param("now") LocalDateTime now,
