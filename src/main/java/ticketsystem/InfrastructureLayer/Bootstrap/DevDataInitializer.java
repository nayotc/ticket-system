...
public class DevDataInitializer implements CommandLineRunner {
    ...
    /**
     * Holds the database-generated identifier of the main development company.
     */
    ...
    /**
     * Creates the main development company and assigns its founder role using
     * the identifier generated by the database.
     */
    private void createTestCompany() {
        System.out.println("Creating test production company...");

        Member founder =
                userRepository.getMemberByUsername(FOUNDER_USERNAME);

        if (founder == null) {
            throw new IllegalStateException(
                    "Cannot create development company because the founder was not found."
            );
        }
        ...
        Company company = companyRepository.findAll()
                ...
                .filter(existingCompany ->
                        COMPANY_NAME.equals(existingCompany.getName()))
                .filter(existingCompany ->
                        existingCompany.getFounderId() == founder.getId())
                .findFirst()
                .orElse(null);
        ...
        if (company == null) {
            ...
            /*
            * The database assigns the company ID during save.
            */
            companyRepository.save(company);
        }

        this.testCompanyId = company.getId();

        /*
        * The founder role can only be created after the company has received
        * its database-generated identifier.
        */
        if (founder.getRoleInCompany(testCompanyId) == null) {
            founder.addFounderRole(testCompanyId);
    ...
    /**
     * Returns the database-generated identifier of the main development company.
     *
     * @return initialized company identifier
     * @throws IllegalStateException if the company has not been created yet
     */
    private long getTestCompanyId() {
        if (testCompanyId == null) {
            throw new IllegalStateException(
                    "Development company has not been initialized yet."
            );
        }
    ...
    private void assignTeamRoles() {
        ...
        Founder founderRole = (Founder) founder.getRoleInCompany(getTestCompanyId());

        // 1. Assign an active Owner role
        owner.addOwnerRole(getTestCompanyId(), founder.getId());
        CompanyRole ownerRole = owner.getRoleInCompany(getTestCompanyId());
        ...
        manager.addManagerRole(getTestCompanyId(), founder.getId(), managerPerms);
        CompanyRole managerRole = manager.getRoleInCompany(getTestCompanyId());
        ...
        pendingUser.addManagerRole(getTestCompanyId(), founder.getId(), pendingPerms);
    ...
    private void createTestEvents() {
        System.out.println("Creating test events for company ID " + getTestCompanyId() + "...");
        ...
        if (owner != null) {
            ...
            long mainCompanyId = getTestCompanyId();
    ...
    private void createTestSalesData() {
        ...
        OrderDTO order2 = new OrderDTO(8491L, List.of(ticket3), "הופעת רוק במדבר", "באר שבע", buyerId, getTestCompanyId(), founderId, 92L, new BigDecimal(100), 222222,false);
        ...
        OrderDTO order3 = new OrderDTO(8493L, List.of(ticket4, ticket5), "פסטיבל שקיעה", "תל אביב", buyerId, getTestCompanyId(), ownerId, 101L, new BigDecimal(100),111111,false);
        ...
        OrderDTO order4 = new OrderDTO(8494L, List.of(ticket6), "הופעת ג'אז", "באר שבע", buyerId, getTestCompanyId(), ownerId, 102L, new BigDecimal(100), 111111,false);
    ...
    private void createReportOnlyManager() {
        ...
        manager.getRoleInCompany(getTestCompanyId()).setStatus(RoleStatus.ACTIVE);
